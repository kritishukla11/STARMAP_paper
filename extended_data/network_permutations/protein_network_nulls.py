...
def parse_args():
    ...
    parser.add_argument("--input_csv", required=True, help="Input CSV with protein/pathway/score columns")
...
def build_projected_graph(df_in, top_k=25):
    ...
    path = df_in["pathway"].astype("category")
...
def null_A_global_pathway_permute(df_in, rng):
    out = df_in.copy()
    out["pathway"] = rng.permutation(out["pathway"].to_numpy())
    return out
...
def main():
    ...
    df = df.dropna(subset=["protein", "pathway", "score"]).copy()
    ...
    obs_df.to_csv(f"{args.output_prefix}_observed_graph_stats.csv", index=False)
    ...
    for i in range(args.n_null):
        ...
        df_A = null_A_global_pathway_permute(df, rng)
        G_A = build_projected_graph(df_A, top_k=args.top_k)
        stats_A = compute_graph_stats(G_A)
        stats_A["null_type"] = "A_global_pathway_permute"
        stats_A["iter"] = i
    ...
    summary_df.to_csv(f"{args.output_prefix}_null_comparison_summary.csv", index=False)

    direction = {
        "weighted_clustering": +1,
        "modularity": +1,
        "lcc_frac": +1,
        "weighted_degree_gini": +1,
        "weighted_degree_cv": +1
    }

    comp_rows = []
    for null_type, sub in summary_df.groupby("null_type"):
        z_list = []
        for _, row in sub.iterrows():
            z = row["z_score"]
            metric = row["metric"]
            if pd.notna(z):
                z_list.append(direction[metric] * z)

        comp_rows.append({
            "null_type": null_type,
            "composite_mean_z": np.mean(z_list) if len(z_list) else np.nan,
            "n_metrics": len(z_list)
        })

    comp_df = pd.DataFrame(comp_rows)
    comp_df.to_csv(f"{args.output_prefix}_composite_scores.csv", index=False)

    print("Done.")
    print(f"Saved: {args.output_prefix}_observed_graph_stats.csv")
    print(f"Saved: {args.output_prefix}_null_graph_stats.csv")
    print(f"Saved: {args.output_prefix}_null_comparison_summary.csv")
    print(f"Saved: {args.output_prefix}_composite_scores.csv")
